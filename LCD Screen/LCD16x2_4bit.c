...
void lcdinit()
{
	LCD_DDDR = 0xFF;	//All pins of LCD_DDDR are output
	_delay_ms(200);									/* WAIT FOR SOME TIME */
	lcdcommand(0x33);		//Function set to 8-bit mode
	lcdcommand(0x32);		//Function set to 8-bit mode again /* SEND $32 FOR INIT OT 0X02 */
	lcdcommand(0x28);		//Set function to 4-bit mode, indicate LCD has two lines. /* INIT. LCD 2 LINE, 5 X 7 MATRIX */
...
void lcd_gotoxy(unsigned char x, unsigned char y)
{
	unsigned char firstcharadd[]={0x80, 0xC0};
		 //0x80=>Force the cursor to the beginning of 1st line
		 // 0xC0=>Force the cursor to the beginning of 2nd line
